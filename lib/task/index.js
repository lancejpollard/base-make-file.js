...
function mintHalt(base) {
  const text = findName(base)
  const halt = {
    form: 'zone-halt',
    text
  }
...
function mintLoan(base) {
  const name = base.link[0].form === 'nest' ? mintNest(base.link[0]) : findName(base)
  const lock = !base.link.slice(1).find(base => base.name === 'lock')
  if (typeof name === 'string') {
    return {
      form: 'loan',
      name,
...
function mintHook(base) {
  ...
  const zone = {
    form: `hook`,
    name,
    bind: null, // pass it a task by name
    link: [],
    head: [],
    bear: [],
    zone: [],
  ...
  base.link.slice(1).forEach(base => {
    switch (base.name) {
      case 'bind':
        zone.bind = findName(base)
        break
      case 'head':
        zone.head.push(mintLink(base))
        break
      case 'bear':
        zone.bear.push(mintLink(base))
        break
      case 'link':
